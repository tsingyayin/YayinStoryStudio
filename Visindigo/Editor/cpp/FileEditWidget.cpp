...
protected:
	...
	*/
	...
	/*!
		\fn Visindigo::Editor::FileEditWidget::fileChanged(const QString& filePath)
		当文件内容被修改时，此信号会被触发。
	*/

	/*!
		\fn Visindigo::Editor::FileEditWidget::fileSaved(const QString& filePath)
		当文件被成功保存时，此信号会被触发。
	*/
...
/*!
	\fn Visindigo::Editor::FileEditWidget::onOpen(const QString& path) = 0
...
/*!
	\fn Visindigo::Editor::FileEditWidget::onClose() = 0
	派生类必须实现此纯虚函数以处理文件关闭逻辑。
	\return 如果文件编辑器可以关闭，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onSave(const QString& path = "") = 0
...
/*!
	\fn Visindigo::Editor::FileEditWidget::onReload() = 0
	派生类必须实现此纯虚函数以处理文件重新加载逻辑。
	\return 如果文件成功重新加载，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onCopy() = 0
	派生类必须实现此纯虚函数以处理复制逻辑。
	\return 如果复制操作成功，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onCut() = 0
	派生类必须实现此纯虚函数以处理剪切逻辑。
	\return 如果剪切操作成功，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onPaste() = 0
	派生类必须实现此纯虚函数以处理粘贴逻辑。
	\return 如果粘贴操作成功，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onUndo() = 0
	派生类必须实现此纯虚函数以处理撤销逻辑。
	\return 如果撤销操作成功，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onRedo() = 0
	派生类必须实现此纯虚函数以处理重做逻辑。
	\return 如果重做操作成功，返回true；否则返回false。
*/

/*!
	\fn Visindigo::Editor::FileEditWidget::onSelectAll() = 0
	派生类必须实现此纯虚函数以处理全选逻辑。
	\return 如果全选操作成功，返回true；否则返回false。
*/