...
#ifndef QT_NO_DEBUG
#define VI_VAR_SIGNAL(name) qFlagLocation(QString::number(QSIGNAL_CODE)+name+QString(QLOCATION).toUtf8().constData())
#define VI_VAR_SLOT(name) qFlagLocation(QString::number(QSLOT_CODE)+name+QString(QLOCATION).toUtf8().constData())
#else
...
namespace Visindigo::Utility {
	...
	void QtSSHelper::connect(QObject* emiter, const QString& signalName, QObject* reciver, const QString& slotName) {
		if (!emiter || !reciver) {
			yDebugF << "emiter or reciver is null";
		}
		...
		if (!emiter || !reciver) {
			yDebugF << "emiter or reciver is null after findChildObject";
		}
		QObject::connect(emiter, VI_VAR_SIGNAL(finalSignalName), reciver, VI_VAR_SLOT(finalSlotName));
	}
}