...
1. AI生成的代码必须经过人工审核，确保其符合项目的编码规范和质量要求。
2. 由AI成段落生成的代码应标注由AI生成，由提交者和维护者共同负责。不标注的视同人工编写，由提交者全权负责。
3. AI生成的代码不得包含对已有ABI的破坏性更改，除非经过充分的测试和验证，并且获得了项目维护者的批准。
4. 包含AI生成的代码的分支，合并时不能与主线相差超过2000行或20个文件（以多者为准），以便于维护者进行人工审核。（这里2000行按add + del计算）
5. AI不得参与API设计，也不得参与功能的架构设计，只能在具体实现层面提供辅助。
